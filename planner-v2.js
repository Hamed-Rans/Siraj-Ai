  ...
  function init(){
    ...
    function stopAudio(){
      if (audioNodes && audioNodes.source){
        ...
        try { audioNodes.source.disconnect(); } catch(e){}
      }
    ...
    function makeNoise(type){
      ...
      if (type === 'nature'){
        ...
        var q = audioCtx.createBiquadFilter();
        q.type = 'peaking'; q.frequency.value = 800; q.gain.value = 5;
        src.connect(q); q.connect(filter);
      } else if (type === 'rain'){
      ...
      src.start();
      audioNodes = {source:src, gain:gain, filter:filter};
      currentSound = type;
      return true;
    }
        ...
        '<div id="studySetup" class="study-setup">' +
          ...
          '<div class="study-music-section">' +
            ...
            '<div class="study-sound-grid">' +
              '<button class="study-sound-btn" data-sound="nature"><span class="ss-icon">🌲</span><span>طبیعت</span></button>' +
              '<button class="study-sound-btn" data-sound="hall"><span class="ss-icon">📚</span><span>سالن مطالعه</span></button>' +
              '<button class="study-sound-btn" data-sound="rain"><span class="ss-icon">🌧️</span><span>بارش</span></button>' +
            ...
            '<div class="study-volume-row">' +
              ...
              '<input type="range" min="0" max="100" value="40" id="studyVolume">' +
            '</div>' +
          '</div>' +
    ...
    function startStudy(){
      ...
      if (currentSound && nowPlaying){
        var names = {nature:'صدای طبیعت 🌲', hall:'سالن مطالعه 📚', rain:'بارش باران 🌧️'};
    ...
    setTimeout(function(){
      ...
      if (aboutContent && !aboutContent.querySelector('.siraj-version-badge')){
        ...
        aboutContent.appendChild(badge);
      }
    }, 2000);
