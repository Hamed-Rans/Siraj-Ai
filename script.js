...
/* ★ توابع سراسری پروفایل و سطح — رفع باگ تنظیمات */
...
function loadSettings(){
    try{
        ...
        return s;
    }catch(e){return{...APP_CONFIG.defaultSettings,models:JSON.parse(JSON.stringify(APP_CONFIG.models))};}
}
...
function selectDropdown(id,value,key,e){
    e.stopPropagation();
    settings[key]=value;saveSettings();applySettingsToUI();
    document.querySelectorAll('.dropdown.open').forEach(d=>d.classList.remove('open'));
...
function saveRename(id,newTitle){
    ...
    h[id].title=t;saveHistory(h);renderHistory();
}
...
function deleteMsg(e,btn){
    ...
    if(i>=0&&currentChatId){
        const h=loadHistory();
        if(h[currentChatId]?.messages[i]){h[currentChatId].messages.splice(i,1);saveHistory(h);renderHistory();}
    }
    w.remove();toggleWelcome();
}
...
async function blobToWav(blob){
    ...
    const ch=[];for(let i=0;i<nCh;i++)ch.push(buf.getChannelData(i));
    let off=44;
    for(let i=0;i<len;i++){
        for(let c=0;c<nCh;c++){
...
async function toggleVoice(){
    ...
    if(isRecording){try{mediaRecorder?.stop();}catch(e){}return;}
    if(!navigator.mediaDevices){toast('مرورگر پشتیبانی نمی‌کنه','error');return;}
    try{
        const s=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true}});
        mediaRecorder=new MediaRecorder(s);recordedChunks=[];
        ...
        mediaRecorder.start();
        isRecording=true;btn.classList.add('recording','active');
...
async function send(){
    ...
    input.value='';currentFile=null;currentFileType=null;
    renderFilePreview();handleInput();
    toggleWelcome();
    chatInFlight=true;
    const box=document.getElementById('box');
...
function handleBgUpload(ev){
    ...
    r.onload=e=>{
        ...
        updatePreview();renderSettingsControls();
...
function clearBgImage(){
    if(!settingsDraft)return;
    settingsDraft.bgImage='';settingsDraft.bgPreset='none';
    updatePreview();renderSettingsControls();
}
